...
--
-- Changelog (v5.1):
--   - Fix: Battery health data no longer resets on disarm or RX loss.
--     Health now persists across the entire battery session and only
--     resets on a confirmed battery swap or manual long-press reset.
--   - Long-press ENTER now resets health + flight timer together.
--   - Added separate resetBattery() vs resetHealthOnly() so swap
--     detection can do a full reset while manual reset is targeted.
...
-- ── Reset functions ──────────────────────────────────────────────
-- Full battery reset: called on confirmed battery swap.
-- Clears everything including cell count, HV detection, etc.
...
-- Health-only reset: called on manual long-press ENTER.
-- Resets flight timer and health data but preserves cell count and
-- HV detection since the same physical battery is still connected.
local function resetHealthOnly()
    armed         = false
    flightTime    = 0
    segStart      = 0
    startV        = nil
    ...
    startedFull   = false
    lastCurrentTick = nil
    -- Deliberately NOT resetting: cellCount, hvLipo, maxV,
    -- baselineCellV, disconnectTime, lastCapaSeen, cellManual
end
...
local function bg_func()
    ...
    if nowArmed and not armed then
        -- ── Arming: accumulate health across flights on same battery ──
        -- Only set startV on the FIRST arm of this battery session.
        ...
        -- Ensure minFlightV has a valid starting point for this arm.
        -- If we already have a minFlightV from a previous arm on this
        -- battery, we keep it — we only set a fresh one if nil.
    ...
    elseif not nowArmed and armed then
        -- ── Disarming: save flight time but KEEP health data ──────
        -- Health (startV, minFlightV, batteryUsed, usedMah, startedFull)
        -- persists so you can review it after landing, crashing, or
        -- losing RX link.  Only a real battery swap or manual
        -- long-press ENTER will reset it.
...
local function run_func(event, touchState)
    ...
    -- Resets health data + flight timer, but preserves cell count
    -- and HV detection since the same battery is still connected.
