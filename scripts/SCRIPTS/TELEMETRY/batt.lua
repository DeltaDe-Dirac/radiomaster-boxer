...
--   5S: 16.00- 22.00V
--   6S: 19.20- 26.40V
-- Bands ordered high-to-low to resolve overlaps correctly.
-- A discharged 6S (19.2V+) would match 5S upper range if checked first.
-- Checking 6S first avoids misidentification.
-- Tightened boundaries: each band uses 3.50V/cell lo, 4.40V/cell hi
-- so a properly charged pack always lands in the right band.
local CELL_BANDS = {
  { cells=6, lo=21.00, hi=26.40 },
  { cells=5, lo=17.50, hi=22.00 },
  { cells=4, lo=14.00, hi=17.60 },
  { cells=3, lo=10.50, hi=13.20 },
  { cells=2, lo=7.00,  hi=8.80  },
  { cells=1, lo=3.20,  hi=4.40  },
}

local function detectCells(packV)
  -- Check from highest cell count down to avoid overlap misdetection
  ...
  return nil  -- unrecognised voltage
end
...
-- ── State ────────────────────────────────────────────────────────
local cellCount   = nil   -- auto-detected, nil until first read
...
local function bg_func()
  ...
  -- Auto-detect cell count once per battery
  if packV and cellCount == nil then
...
local function run_func()
  ...
  local cellStr = cellCount and (cellCount .. "S") or "?S"
